...
const Title: React.FC<{ text: string; color: string; accentColor: string }> = ({ text, color, accentColor }) => (
  <h3 className="text-base font-bold mb-3 pb-1 uppercase tracking-wide" style={{ color: color, borderBottom: `1px solid ${accentColor}` }}>
    {text}
  </h3>
);
      ...
      <div className="h-full grid grid-cols-[1.5fr_3.5fr] gap-x-8 p-8"> {/* Two-column layout with padding */}
        ...
        <div className="flex flex-col pr-8 space-y-8 border-r border-gray-200"> {/* Added border-r */}
          ...
          {(resumeData.contactInfo.linkedin || resumeData.contactInfo.github || resumeData.contactInfo.website) && (
            <div className="resume-section">
              <Title text="Links" color={mainText} accentColor={accentColor} />
          ...
          {hasArrayData(resumeData.languages) && (
            <div className="resume-section">
              <Title text="Languages" color={mainText} accentColor={accentColor} />
          ...
          {hobbies.length > 0 && (
            <div className="resume-section">
              <Title text="Hobbies" color={mainText} accentColor={accentColor} />
        ...
        <div className="flex flex-col space-y-8">
          ...
          <div className="flex-grow-0"> {/* Prevent this section from growing */}
            ...
            <h2 className="text-lg font-medium mb-4 uppercase tracking-widest" style={{ color: subtleText }}>
          ...
          {resumeData.profileInfo.summary && (
            <div className="resume-section">
              <Title text="About Me" color={mainText} accentColor={accentColor} />
                  ...
                  <div key={index} className="flex items-start gap-2"> {/* Adjusted gap */}
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between items-baseline">
                        <h4 className="text-sm font-bold" style={{ color: mainText }}>{exp.role || "Role"} at {exp.company || "Company"}</h4>
              ...
              <div className="space-y-4">
                {resumeData.education.map((edu, index) => (
                  <div key={index} className="flex items-start gap-2"> {/* Adjusted gap */}
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between items-baseline">
                        <h4 className="text-sm font-bold" style={{ color: mainText }}>{edu.degree || "Degree"}</h4>
              ...
              <div className="space-y-4">
                {resumeData.projects.map((project, index) => (
                  <div key={index} className="flex items-start gap-2"> {/* Adjusted gap */}
                    <Circle className="w-4 h-4 flex-shrink-0 mt-1" style={{ color: accentColor }} />
                    <div className="flex-1">
                      <h4 className="text-sm font-bold" style={{ color: mainText }}>{project.title || "Project Title"}</h4>
                        ...
                        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs mt-2">
                          {project.github && (
                            <a href={project.github} target="_blank" rel="noopener noreferrer" className="hover:underline flex items-center gap-1" style={{ color: accentColor }}>
                              <Github className="w-3 h-3" /> {cleanUrlForDisplay(project.github)}
                            </a>
                          )}
                          {project.LiveDemo && (
                            <a href={project.LiveDemo} target="_blank" rel="noopener noreferrer" className="hover:underline flex items-center gap-1" style={{ color: accentColor }}>
                              <ExternalLink className="w-3 h-3" /> {cleanUrlForDisplay(project.LiveDemo)}
                            </a>
                          )}
                        </div>
                      )}
